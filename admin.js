...
function renderOrders() {
  ...
  currentOrders.forEach(order => { const node = template.content.cloneNode(true); const card = node.querySelector('.order-card'); card.dataset.id = order.id; node.querySelector('.service').textContent = order.service; const status = node.querySelector('.status'); status.textContent = order.status; status.classList.add(statusClass(order.status)); node.querySelector('.idea').textContent = order.idea; node.querySelector('.meta').textContent = `${order.email} · ${order.payment} · ${order.price} · ${order.size} · ${order.style}`; node.querySelector('.date').textContent = `订单号 ${order.id} · ${order.date}`; const select = node.querySelector('.status-select'); select.value = order.status; select.disabled = order.status === '已交付'; select.addEventListener('change', event => updateOrder(order, event.target.value)); node.querySelector('.file-input').addEventListener('change', event => deliver(order, event.target.files[0])); list.appendChild(node); });
}
...
async function updateOrder(order, status) { if (status === '已交付') { alert('请先上传成品，系统会自动邮件交付并标记为已交付。'); return; } try { if (order.status !== status && ['审核中', '制作中'].includes(status)) await api('/api/notify-status', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email: order.email, orderId: order.id, service: order.service, price: order.price, status }) }); await api(`/api/orders?id=${encodeURIComponent(order.id)}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ status }) }); await loadOrders(); setNotice(status === '制作中' ? '已更新为制作中，并已向客户发送进度邮件。' : '订单状态已更新。'); } catch (error) { alert(`更新失败：${error.message}`); await loadOrders(); } }

...
export default async function balanceHandler(req, res) {
  ...
  const recharges = (await Promise.all((ids || []).map(loadRecharge)))
    ...
    .slice(0, 10)
    .map(item => ({ id: item.id, amount: item.amount, creditedAmount: item.creditedAmount, payment: item.payment, status: item.status, requestedAt: item.requestedAt, approvedAt: item.approvedAt || '' }));

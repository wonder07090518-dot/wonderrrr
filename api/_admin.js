...
export function configured() { return Boolean(process.env.ADMIN_EMAIL && process.env.ADMIN_PASSWORD && secret()); }
export function storageConfigured() { return Boolean(process.env.KV_REST_API_URL && process.env.KV_REST_API_TOKEN); }
export function issueSession(res, email) { const payload = Buffer.from(JSON.stringify({ email, expires: Date.now() + 1000 * 60 * 60 * 12 })).toString('base64url'); const token = `${payload}.${signature(payload)}`; res.setHeader('Set-Cookie', `${COOKIE}=${token}; Path=/; HttpOnly; Secure; SameSite=Strict; Max-Age=43200`); }
export function clearSession(res) { res.setHeader('Set-Cookie', `${COOKIE}=; Path=/; HttpOnly; Secure; SameSite=Strict; Max-Age=0`); }
export function isAdmin(req) { try { const token = cookies(req)[COOKIE]; if (!token) return false; const [payload, received] = token.split('.'); if (!payload || !received || !matches(received, signature(payload))) return false; const session = JSON.parse(Buffer.from(payload, 'base64url').toString()); return session.email === process.env.ADMIN_EMAIL && session.expires > Date.now(); } catch { return false; } }
export function validCredentials(email, password) { return Boolean(process.env.ADMIN_EMAIL && process.env.ADMIN_PASSWORD && matches(email, process.env.ADMIN_EMAIL) && matches(password, process.env.ADMIN_PASSWORD)); }
